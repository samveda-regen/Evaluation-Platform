import './env.js'; // must stay the first import — see src/env.ts for why
...
import { getExecutionQueueStatus } from './utils/codeExecutor.js';
import { startTestGradingWorker, stopTestGradingWorker } from './workers/testGradingWorker.js';
import { startProctorAnalysisWorker, stopProctorAnalysisWorker } from './workers/proctorAnalysisWorker.js';
...
// In-process workers: their job bodies call emitToTestProctorRoom /
// emitToProctorTargets, which need the live socket.io instance set above —
// that's why these run inside the API process rather than as standalone
// scripts like codeExecutionWorker.ts.
startTestGradingWorker();
startProctorAnalysisWorker();
...
app.get('/api/health/queues', async (_req, res) => {
  try {
    const codeExecution = await getExecutionQueueStatus();
    res.json({
      status: 'ok',
      timestamp: new Date().toISOString(),
      codeExecution,
    ...
    res.status(503).json({
      status: 'degraded',
      error: error instanceof Error ? error.message : 'Unable to reach Redis',
      timestamp: new Date().toISOString(),
    });
  }
});
...
async function gracefulShutdown(signal: string): Promise<void> {
  ...
  try {
    await Promise.all([stopTestGradingWorker(), stopProctorAnalysisWorker()]);
  } catch (err) {
