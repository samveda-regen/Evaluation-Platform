...
function riskCfg(row: TrustReportRow) {
  ...
  if (row.riskLevel === 'medium')
    return { label:'Review', color:'#D97706', dot:'#F59E0B', bar:'#F59E0B', ring:'#F59E0B', bg:'#FFF6EE' };
  return { label:'Trusted', color:'#D97706', dot:'#F59E0B', bar:'#F59E0B', ring:'#F59E0B', bg:'#FFF6EE' };
}
...
/* ── SVG Donut ── */
function DonutRing({ pct, size=100, sw=10, color='#F59E0B' }: { pct:number; size?:number; sw?:number; color?:string }) {
  const r    = (size - sw) / 2;
...
function KPICard({ icon, iconBg, value, label }: { icon:React.ReactNode; iconBg:string; value:string|number; label:string }) {
  return (
    <div className="stat-card" style={{
      ...
      <p style={{ fontSize:'28px', fontWeight:700, color:'#11162A', margin:'0 0 4px', lineHeight:1.1 }}>{value}</p>
      <p style={{ fontSize:'13px', color:'#6A7387', margin:0 }}>{label}</p>
    </div>
  );
}
    ...
    <div style={{ padding:'0', backgroundColor:'#F9FAFB', minHeight:'100%' }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'24px' }}>
        <div style={{ display:'flex', alignItems:'flex-start', gap:'12px' }}>
          <BackButton />
          <div>
            <h1 style={{ fontSize:"32px", fontWeight:700, letterSpacing:"-0.02em", color:"#11162A", margin:"0 0 4px", lineHeight:1.2 }}>Trust &amp; Integrity Reports</h1>
            <p style={{ fontSize:'13px', color:'#6A7387', margin:0 }}>AI-detected violations and trust scoring across all attempts.</p>
          </div>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:'10px', flexShrink:0 }}>
          ...
          <div style={{ position:'relative', zIndex:20 }}>
            <button onClick={() => setShowTestDropdown(p=>!p)}
              ...
              <ChevronDown size={12} color="#6A7387" />
            </button>
            {showTestDropdown && (
              <div style={{
                ...
                <button onClick={() => handleTestSelect('')}
                  style={{
                    ...
                    backgroundColor: !testIdParam ? '#FFF6EE' : 'white',
                    color: !testIdParam ? '#D97706' : '#434B5E',
                ...
                {testTree.map(t => (
                  <button key={t.id} onClick={() => handleTestSelect(t.id)}
                    style={{
                      width:'100%', textAlign:'left', padding:'10px 14px', border:'none',
                      backgroundColor: testIdParam===t.id ? '#FFF6EE' : 'white',
                      color: testIdParam===t.id ? '#D97706' : '#434B5E',
                      fontSize:'13px', cursor:'pointer', borderBottom:'1px solid #F3F4F6',
                    }}>
                    <p style={{ margin:'0 0 2px', fontWeight:500 }}>{t.name}</p>
                    <p style={{ margin:0, fontSize:'11px', color:'#98A2B5' }}>{t.testCode} · {t.attempts} attempts</p>
                  </button>
                ))}
              </div>
            )}
      ...
      {loading ? (
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 360px', gap:'16px', alignItems:'start' }}>
          ...
          <div style={{ backgroundColor:'white', borderRadius:'14px', boxShadow:'0 1px 6px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ padding:'20px 22px 0' }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'14px' }}>
                <p style={{ fontSize:'15px', fontWeight:600, color:'#11162A', margin:0 }}>Flagged attempts</p>
                <div style={{ display:'flex', gap:'6px' }}>
                  ...
                  <button onClick={() => void handleReEvaluateAll()} disabled={bulkReEvalLoading || !reports.length}
                    style={{ padding:'5px 12px', borderRadius:'7px', border:'1px solid #E5E7EB', backgroundColor:'white', fontSize:'12px', color:'#434B5E', cursor:'pointer' }}>
              ...
              <div style={{ display:'flex', gap:'8px', marginBottom:'16px' }}>
                <div style={{ position:'relative', flex:1 }}>
                  <Search size={14} color="#98A2B5" style={{ position:'absolute', left:'10px', top:'50%', transform:'translateY(-50%)', pointerEvents:'none' }} />
                  <input value={search} onChange={e=>setSearch(e.target.value)}
                    onKeyDown={e => e.key==='Enter' && void applySearch()}
                    placeholder="Search candidate, test…"
                    style={{ width:'100%', padding:'8px 12px 8px 32px', borderRadius:'8px', border:'1px solid #E5E7EB', backgroundColor:'white', fontSize:'13px', color:'#434B5E', outline:'none', boxSizing:'border-box' }}
                  />
                </div>
                <CustomSelect
                  value={risk}
                ...
                <button onClick={()=>void applySearch()}
                  style={{ padding:'8px 14px', borderRadius:'8px', border:'none', backgroundColor:'#F59E0B', color:'white', fontSize:'12px', fontWeight:600, cursor:'pointer' }}>
                  Search
                </button>
                {(search || risk || flaggedOnly) && (
                  <button onClick={()=>void clearFilters()}
                    style={{ padding:'8px 12px', borderRadius:'8px', border:'1px solid #E5E7EB', backgroundColor:'white', color:'#6A7387', fontSize:'12px', cursor:'pointer' }}>
                    Clear
                  </button>
                )}
              </div>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:'6px', marginBottom:'12px' }}>
                <label style={{ display:'flex', alignItems:'center', gap:'6px', fontSize:'12px', color:'#6A7387', cursor:'pointer' }}>
                  <input type="checkbox" checked={flaggedOnly} onChange={e=>setFlaggedOnly(e.target.checked)} />
                  Flagged only
                </label>
                <span style={{ fontSize:'12px', color:'#98A2B5', marginLeft:'auto' }}>{sortedReports.length} reports</span>
              </div>
            </div>

            {/* ── Candidate rows ── */}
            {sortedReports.length === 0 ? (
              <p style={{ textAlign:'center', padding:'48px 0', color:'#98A2B5', fontSize:'13px' }}>No trust reports found</p>
            ) : (
                  ...
                  return (
                    <div key={row.attemptId}
                      ...
                      style={{
                        ...
                        backgroundColor: isSel ? '#FFF6EE' : 'white',
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        <p style={{ fontSize:'13px', fontWeight:600, color:'#11162A', margin:'0 0 2px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                          {row.candidateName || row.candidateEmail}
                        </p>
                        <p style={{ fontSize:'11px', color:'#98A2B5', margin:0, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                          {row.testName}
                        </p>
                      </div>
                      {/* risk badge */}
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:'6px', flexShrink:0 }}>
                        ...
                        <span style={{ fontSize:'15px', fontWeight:700, color:'#11162A', minWidth:'28px', textAlign:'right' }}>
          ...
          <div style={{ backgroundColor:'white', borderRadius:'14px', boxShadow:'0 1px 6px rgba(0,0,0,0.06)', overflow:'hidden' }}>
            {!selectedRow ? (
              <div style={{ padding:'48px 20px', textAlign:'center', color:'#98A2B5', fontSize:'13px' }}>
                ...
                <>
                  ...
                  <div style={{ padding:'20px 22px', borderBottom:'1px solid #F3F4F6' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        <p style={{ fontSize:'15px', fontWeight:600, color:'#11162A', margin:'0 0 2px' }}>
                          {selectedRow.candidateName}
                        </p>
                        <p style={{ fontSize:'12px', color:'#98A2B5', margin:0, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                  ...
                  <div style={{ padding:'20px 22px', display:'flex', alignItems:'center', gap:'24px', borderBottom:'1px solid #F3F4F6' }}>
                    ...
                    <div style={{ position:'relative', flexShrink:0 }}>
                      ...
                      <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center' }}>
                        <span style={{ fontSize:'18px', fontWeight:700, color:'#11162A', lineHeight:1 }}>{Math.round(selectedRow.trustScore)}</span>
                        <span style={{ fontSize:'9px', color:'#98A2B5', letterSpacing:'0.06em', marginTop:'2px' }}>TRUST</span>
                      </div>
                    </div>
                    {/* Violations + session link */}
                    <div style={{ flex:1 }}>
                      <p style={{ fontSize:'12px', color:'#98A2B5', margin:'0 0 4px' }}>Detected violations</p>
                      <p style={{ fontSize:'32px', fontWeight:700, color:'#11162A', margin:'0 0 10px', lineHeight:1 }}>
                  ...
                  <div style={{ padding:'16px 22px', borderBottom:'1px solid #F3F4F6' }}>
                    <p style={{ fontSize:'10px', fontWeight:700, color:'#98A2B5', letterSpacing:'0.08em', margin:'0 0 12px' }}>
                      EVIDENCE TIMELINE
                    </p>
                    {timeline.length === 0 ? (
                      <p style={{ fontSize:'12px', color:'#98A2B5' }}>No violation events recorded</p>
                    ) : (
                      <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
                        {timeline.map((item, i) => (
                          <div key={i} style={{ display:'flex', alignItems:'center', gap:'10px' }}>
                            <CameraIcon />
                            <div style={{ flex:1, minWidth:0 }}>
                              <p style={{ fontSize:'13px', fontWeight:500, color:'#11162A', margin:'0 0 1px' }}>{item.label}</p>
                              {item.timeStr && (
                                <p style={{ fontSize:'11px', color:'#98A2B5', margin:0 }}>{item.timeStr} · evidence captured</p>
                              )}
                            </div>
                            {/* snapshot circle */}
                            <button
                              ...
                              {item.snapshotUrl ? (
                                ...
                                <span style={{ fontSize:'8px', color:'#98A2B5' }}>○</span>
                              )}
                            </button>
                          </div>
                        ))}
                  ...
                  <div style={{ padding:'10px 22px', borderBottom:'1px solid #F3F4F6', display:'flex', justifyContent:'flex-end' }}>
                    <button onClick={()=>void handleReEvaluate(selectedRow!.attemptId)}
                      ...
                      style={{ padding:'6px 14px', borderRadius:'7px', border:'1px solid #E5E7EB', backgroundColor:'white', fontSize:'12px', color:'#434B5E', cursor:'pointer' }}>
                  ...
                  <div style={{ padding:'16px 22px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px' }}>
                    <button onClick={()=>void handleMarkTrusted()} disabled={markTrustedLoading}
                      style={{
                        ...
                        backgroundColor:'white', fontSize:'13px', fontWeight:500, color:'#434B5E',
        ...
        <div style={{
          ...
          <div style={{ backgroundColor:'white', borderRadius:'16px', width:'100%', maxWidth:'900px', maxHeight:'90vh', overflow:'hidden', display:'flex', flexDirection:'column' }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px 20px', borderBottom:'1px solid #F3F4F6' }}>
              <div>
                <p style={{ fontSize:'16px', fontWeight:600, color:'#11162A', margin:'0 0 2px' }}>
                  Violation Proofs: {activeProofRow.candidateName}
                </p>
                <p style={{ fontSize:'12px', color:'#6A7387', margin:0 }}>
            ...
            <div style={{ padding:'16px', overflowY:'auto', flex:1 }}>
              {(activeProofRow.violationProofs||[]).length === 0 ? (
                <p style={{ textAlign:'center', color:'#98A2B5', fontSize:'13px', padding:'40px 0' }}>No evidence images available.</p>
              ) : (
                    ...
                    <div key={idx} style={{ borderRadius:'10px', border:'1px solid #E5E7EB', overflow:'hidden' }}>
                      ...
                      <div style={{ padding:'10px 12px' }}>
                        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'4px' }}>
                          ...
                          <span style={{ fontSize:'10px', padding:'2px 8px', borderRadius:'20px', backgroundColor: proof.isAiEvent?'#FFF6EE':'#F3F4F6', color: proof.isAiEvent?'#92400E':'#6B7280' }}>
                            {proof.isAiEvent?'AI':'Non-AI'}
                          </span>
                        </div>
                        <p style={{ fontSize:'11px', color:'#98A2B5', margin:'2px 0' }}>Severity: {proof.severity}</p>
                        {proof.timestamp && <p style={{ fontSize:'11px', color:'#98A2B5', margin:'2px 0' }}>{format(new Date(proof.timestamp),'MMM d, h:mm:ss a')}</p>}
